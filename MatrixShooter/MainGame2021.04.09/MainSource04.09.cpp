...
void progress_space(unsigned char*** fspace, c_u_char space_depth, c_u_char space_height, c_u_char space_width, c_u_char player_x, c_u_char player_y, long& distance_travelled);
...
bool check_player_status(u_char*** fspace, entity player);
...
int main()
{
	...
	while (true) {
		...
		if (check_player_status(space, player)) {
			exit(0); //replace with death screen
		}
		progress_space(space, space_depth, space_height, space_width, player.x_coord, player.y_coord, distance_travelled); //0.022 ms latest exec time
	}
...
void print_space_visible3(unsigned char*** fspace, c_u_char space_height, c_u_char space_width, entity& player) {
	...
	for (index_height = 0; index_height < space_height; ++index_height) {
		if (index_height != player.y_coord) { //if we are not writing the line on which the player is located we use fputs for efficiency
			...
			if (fspace[0][player.y_coord][player.x_coord] == ROCK) {
				SetConsoleTextAttribute(hConsole, FONT_COLOR_ROCK);
			}
			else if (fspace[1][player.y_coord][player.x_coord] == ROCK) {
				SetConsoleTextAttribute(hConsole, FONT_COLOR_ROCK_CLOSE);
			}
			else if (fspace[2][player.y_coord][player.x_coord] == ROCK) {
	...
	printf("\n\n0: %c\n1: %c\n2: %c", fspace[0][player.y_coord][player.x_coord], fspace[1][player.y_coord][player.x_coord], fspace[2][player.y_coord][player.x_coord]);
}
void check_for_keystroke(char& fkeystroke) {
